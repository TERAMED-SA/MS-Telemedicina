...
export class HttpClient {
  ...
  get<T>(url: string): Observable<T> {
    return this.httpService.get<T>(url, { headers: { 'Content-Type': undefined } })
      .pipe(map((response: AxiosResponse<T>) => response.data));
  }

  post<T>(url: string, data: any): Observable<T> {
    return this.httpService.post<T>(url, data, { headers: { 'Content-Type': undefined } }).pipe(map((response: AxiosResponse<T>) => response.data));
  }
